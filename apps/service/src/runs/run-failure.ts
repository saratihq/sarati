/** The structured payload a failed run's error carries, so the failure is debuggable without a second lookup. */
...
/** The node whose failure ended the run — a tolerated (continue-on-fail) error is not it. */
